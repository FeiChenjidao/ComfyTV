...
import CanvasBanner from '@agent/components/graph/CanvasBanner.vue'
import Button from '@agent/components/ui/button/Button.vue'
import type { LGraphCanvas } from '@agent/lib/litegraph/src/litegraph'
import { app } from '@agent/scripts/app'
import { createNodeLocatorId } from '@agent/types/nodeIdentification'
import { getNodeByLocatorId } from '@agent/utils/graphTraversalUtil'
import { createPositionBounds } from '@agent/utils/positionBounds'
...
const presentation = computed(() => ({
  ...
  accent: isComplete.value
    ? 'ctv:border-l-success-background'
    : 'ctv:border-l-base-foreground',
  iconClass: isComplete.value
    ? 'ctv:icon-[lucide--check] ctv:text-success-background'
  ...
  <Teleport v-if="teleportTarget" :to="teleportTarget">
    ...
    >
      ...
      >
        ...
        <template #actions>
          <div class="ctv:flex ctv:min-w-0 ctv:items-center ctv:gap-1">
            <Button
            ...
            >
              <i class="ctv:icon-[lucide--x] ctv:size-4" aria-hidden="true" />
            </Button>
          </div>
        </template>
      </CanvasBanner>
