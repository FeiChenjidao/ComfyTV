...
import { useCurrentUser } from '@agent/composables/auth/useCurrentUser'
import { useTelemetry } from '@agent/platform/telemetry'
...
import type { LGraphCanvas, LGraphNode } from '@agent/lib/litegraph/src/litegraph'
import { useAppMode } from '@agent/composables/useAppMode'
import { MIME_ASSET_INFO } from '@agent/platform/assets/schemas/mediaAssetSchema'
import {
  fetchDroppedAsset,
  getDroppedAsset,
  hasVideoType
...
import { useAgentNodeSelectionStore } from '@agent/stores/agentNodeSelectionStore'
import { useExecutionErrorStore } from '@agent/stores/executionErrorStore'
import { useWorkflowTabActivityStore } from '@agent/stores/workflowTabActivityStore'
import { useSidebarTabStore } from '@agent/stores/workspace/sidebarTabStore'
import { isLGraphNode } from '@agent/utils/litegraphUtil'
import { useToastStore } from '@agent/platform/updates/common/toastStore'
import { toOwningGraphId, toRootGraphId } from '@agent/types/graphScopeId'
import { isCloud } from '@agent/platform/distribution/types'
import { parseNodeId } from '@agent/types/nodeId'
import { useBillingContext } from '@agent/composables/billing/useBillingContext'
import { useAccountPreconditionDialog } from '@agent/platform/cloud/subscription/composables/useAccountPreconditionDialog'
import { useBillingCapabilities } from '@agent/platform/workspace/composables/useBillingCapabilities'
import { useOnboardingTourStore } from '@agent/platform/onboarding/onboardingTourStore'
import { useWorkspaceUI } from '@agent/platform/workspace/composables/useWorkspaceUI'
import { useTeamWorkspaceStore } from '@agent/platform/workspace/stores/teamWorkspaceStore'
import {
...
watch(
  ...
  (graph, _previous, onCleanup) => {
    ...
    events.addEventListener('ctv:node:removed', onNodeRemoved)
    onCleanup(() => events.removeEventListener('ctv:node:removed', onNodeRemoved))
  },
  { immediate: true }
)
