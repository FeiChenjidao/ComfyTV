...
export function getDroppedAsset(dataTransfer: DataTransfer): DroppedAsset | undefined {
  ...
  return uri || ref
    ? { name: name ?? ref ?? uri!, uri, ref, kind: asset?.media_kind, previewUrl: asset?.preview_url }
    : undefined
}

export async function fetchDroppedAsset({ name, uri }: DroppedAsset): Promise<File | undefined> {
