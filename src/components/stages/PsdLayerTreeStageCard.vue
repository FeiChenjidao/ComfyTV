<template>
  ...
  >
    ...
    <button type="button" :class="btnClass" @click="fileInput?.click()">
      <i class="pi pi-folder-open" />
      <span class="ctv:truncate">{{ fileName || $t('psdLayerTree.pick') }}</span>
    </button>

    <div
      class="psd-tree ctv:flex-1 ctv:min-h-[120px] ctv:max-h-[160px] ctv:overflow-y-auto ctv:rounded-md
    ...
    >
      <div
        ...
        class="ctv:flex ctv:h-full ctv:min-h-[120px] ctv:items-center ctv:justify-center
               ctv:text-2xs ctv:text-muted-foreground"
      >
        {{ dragOver ? $t('psdLayerTree.dropHint') : $t('psdLayerTree.pickHint') }}
      </div>
      <button
    ...
    <button type="button" :class="btnClass" @click="expandImages">
      <i class="pi pi-sitemap" />
      <span class="ctv:truncate">{{ $t('psdLayerTree.expand') }}</span>
    </button>
    ...
    <div class="ctv:text-2xs ctv:text-center ctv:py-0.5">
      ...
      <span v-else class="ctv:text-muted-foreground">{{ $t('psdLayerTree.pickHint') }}</span>
      <span
...
import { usePsdLayerTree } from '@/composables/stages/usePsdLayerTree'
import { t } from '@/i18n'
...
} = usePsdLayerTree(props.node, props.state)
