            ...
            inputs=[
                *_standard_stage_inputs(),
                ...
                io.Int.Input("batch_size", default=1, min=1, max=8, step=1,
                             ...
                             tooltip="How many images to generate per Run. "
                                     "If the workflow binds this to EmptyLatent/sampler "
                                     "batch, one graph run emits N samples. If the "
                                     "workflow contains ComfyTV Batch Loop (and binds "
                                     "option:batch_size there), the graph runs N times "
                                     "and outputs are merged."),
                _main_prompt_input(tooltip="Primary prompt — the user's intent for this stage. Upstream text inputs are treated as additional context."),
                io.Autogrow.Input("texts",  template=_text_template(8)),
...
class Model3DStage(io.ComfyNode):
            ...
            inputs=[
                *_standard_stage_inputs(),
                ...
                io.Int.Input("seed", default=0, min=0, max=0xFFFFFFFFFFFFFFFF, step=1,
                             tooltip="Seed for Tripo model_seed / Rodin seed (bind option:seed)."),
                io.String.Input("negative", default="", multiline=True, optional=True,
                                tooltip="Negative prompt (Tripo text-to-model). Bind option:negative."),
                io.Boolean.Input("texture", default=True,
                                 tooltip="Tripo: generate texture maps."),
                io.Boolean.Input("pbr", default=True,
                                 tooltip="Tripo: PBR material maps (requires texture)."),
                io.Combo.Input("texture_quality", options=MODEL3D_TEXTURE_QUALITIES, default="standard",
                               tooltip="Tripo texture quality (standard / detailed / extreme)."),
                io.Combo.Input("geometry_quality", options=MODEL3D_GEOMETRY_QUALITIES, default="standard",
                               tooltip="Tripo geometry quality (standard / detailed)."),
                io.Combo.Input("model_version", options=MODEL3D_MODEL_VERSIONS,
                               default=MODEL3D_MODEL_VERSIONS[0],
                               tooltip="Tripo model version."),
                io.Int.Input("face_limit", default=-1, min=-1, max=2000000, step=1,
                             tooltip="Tripo face limit (-1 = provider default)."),
                io.Boolean.Input("quad", default=False,
                                 tooltip="Tripo: quad mesh (FBX; GLB may be empty)."),
                io.Boolean.Input("smart_low_poly", default=False,
                                 tooltip="Tripo: smart low-poly topology."),
                io.Boolean.Input("auto_size", default=True,
                                 tooltip="Tripo: scale textured models to real-world size."),
                io.Combo.Input("orientation", options=MODEL3D_ORIENTATIONS, default="default",
                               tooltip="Tripo image-to-model orientation."),
                io.Combo.Input("texture_alignment", options=MODEL3D_TEXTURE_ALIGNMENTS,
                               default="original_image",
                               tooltip="Tripo texture alignment."),
                io.Combo.Input("material", options=MODEL3D_MATERIALS, default="PBR",
                               tooltip="Rodin material (PBR / Shaded / …)."),
                io.Combo.Input("mode", options=MODEL3D_MODES, default="Regular",
                               tooltip="Rodin Gen-2.5 generation mode (Regular / Fast / Extreme-High)."),
                io.Combo.Input("polygon_count", options=MODEL3D_POLYGON_COUNTS, default="Default",
                               tooltip="Rodin polygon / mesh density preset."),
                io.Combo.Input("geometry_file_format", options=MODEL3D_GEOMETRY_FORMATS, default="glb",
                               tooltip="Rodin Gen-2.5 output format."),
                io.Combo.Input("texture_mode", options=MODEL3D_TEXTURE_MODES, default="Default",
                               tooltip="Rodin Gen-2.5 texture mode."),
                io.Boolean.Input("tapose", default=False,
                                 tooltip="Rodin T/A pose for human-like models (bind option:tapose → TAPose)."),
                io.Boolean.Input("hd_texture", default=False,
                                 tooltip="Rodin Gen-2.5 HD texture."),
                io.Boolean.Input("texture_delight", default=False,
                                 tooltip="Rodin Gen-2.5 remove baked lighting from textures."),
                io.Boolean.Input("addon_highpack", default=False,
                                 tooltip="Rodin Gen-2.5 HighPack addon."),
                _main_prompt_input(tooltip="Primary prompt — the user's intent for this stage. Upstream text inputs are treated as additional context."),
    ...
    async def execute(cls, force_run_token=0, project_id="", parent_output_id=0,
                      workflow="", seed=0, negative="", texture=True, pbr=True,
                      texture_quality="standard", geometry_quality="standard",
                      model_version="", face_limit=-1, quad=False,
                      smart_low_poly=False, auto_size=True,
                      orientation="default", texture_alignment="original_image",
                      material="PBR", mode="Regular", polygon_count="Default",
                      geometry_file_format="glb", texture_mode="Default",
                      tapose=False, hd_texture=False, texture_delight=False,
                      addon_highpack=False,
                      main_prompt="", texts=None, images=None,
        ...
        payload = await invoke_runner(
            ...
            options={
                ...
                'model_version': model_version or MODEL3D_MODEL_VERSIONS[0],
                'face_limit': int(face_limit if face_limit is not None else -1),
                'quad': bool(quad),
                'smart_low_poly': bool(smart_low_poly),
                'auto_size': bool(auto_size),
                'orientation': orientation or 'default',
                'texture_alignment': texture_alignment or 'original_image',
                'material': material or 'PBR',
                'mode': mode or 'Regular',
                'polygon_count': polygon_count or 'Default',
                'geometry_file_format': geometry_file_format or 'glb',
                'texture_mode': texture_mode or 'Default',
                'tapose': bool(tapose),
                'hd_texture': bool(hd_texture),
                'texture_delight': bool(texture_delight),
                'addon_highpack': bool(addon_highpack),
            },
            custom_params=custom_params,
        )
        return _stage_emit_auto(cls, project_id=project_id, payload_str=payload,
