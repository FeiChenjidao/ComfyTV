...
- **Tripo** (`tripo.json` + `_preset.json`) — front/left/back/right views → **mesh** (GLB or FBX). Uses the Comfy Org Tripo API; `File3DFirst` picks the populated Tripo output so Stage `quad` on/off both work. Front required; side views optional.
