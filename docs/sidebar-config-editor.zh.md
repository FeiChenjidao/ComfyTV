...
- **值编辑器** — 该 widget 原本的输入控件(数字框 / 文本框 / 下拉 / 开关)。**绑定到** 为 `(use this value)` 时,直接改这里 = 把工作流出厂值替换成常量。绑到 stage 的 **option / computed** 时,同一控件用来设置**兜底默认值**(stage 控件为空时生效)。上游图片/遮罩绑定仍会灰掉编辑器(值来自连线)。
- **绑定到:** , 下一节讲。
